...
static void render_function_keys(SDL_Renderer *r, int bottom_reserved) {
    ...
    const char *keys = rw >= 1050
        ? "  F1=joy  F2=tape play  F3=rewind  F4=shot  F5=reset  F6=GIF  F7=pause  F8=monitor  F9=options  F10=40/80  F11=full  F12=quit"
        : "  F1 Joy F2 Play F3 Rew F4 Shot F5 Reset F6 GIF F7 Pause F8 Mon F9 Opt F10 40/80 F11 Full F12 Quit";
